...
/**
 * Cantonese-to-written-Chinese system prompt.
 * Instructs the LLM to correct speech-recognition errors, then convert Cantonese
 * wording into natural Traditional Chinese prose suitable for Hong Kong primary students.
 */
...
2. 將廣東話口語轉換為自然、清楚、符合香港小學生閱讀程度的標準中文書面語，並使用繁體中文。
3. 保留原意，不增加原文沒有的人名、事實或情節。請把「我今日去咗學校」轉為「我今天去了學校」。
4. 加上適當的標點符號，但絕對不可輸出感嘆號（! 或 ！）；若需要句末標點，請使用句號、問號或其他合適標點。
5. 只輸出修正及轉換後的最終文字，絕對不要包含任何解釋、問候語或引號。
...
/**
 * Correct Cantonese text and convert it to written Chinese using AI.
 * If correction fails, return the original text with exclamation marks removed.
 */
async function convertToWrittenChinese(text: string): Promise<string> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      const response = (await Promise.race([
        invokeLLM({
          messages: [
            ...
            { role: 'user', content: `請修正並將以下廣東話文字轉換為標準中文書面語：\n${text}` },
          ],
          model: 'gpt-4o-mini',
        }),
      ...
      return removeExclamationMarks(corrected);
    } catch (error) {
  ...
  return removeExclamationMarks(text);
}
...
export const voiceRouter = router({
  transcribe: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const text = removeExclamationMarks(result.text.trim());
        if (!text) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
  ...
  correct: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const corrected = await convertToWrittenChinese(input.text);
        return {
          original: input.text,
          corrected,
          success: true,
